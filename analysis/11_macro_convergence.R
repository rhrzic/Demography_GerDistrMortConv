...
supp_figA6 = ggplot(filter(e0_var_sens, measure %in% c('variance', 'w.variance')), 
                    aes(x = year, y = dispersion, group = measure, color = measure))+
  geom_point(aes(shape = measure), show.legend = F)+
  geom_smooth(aes(linetype = measure), se = FALSE, show.legend = F)+
  xlab("Year")+
  ylab(expression(paste("Variance of ", e[0])))+
  theme_bw()+
  facet_wrap(. ~ str_to_title(sex), scales = 'free')+
  scale_color_manual(values = c('variance' = 'black', 'w.variance' = 'red'))+
  scale_shape_manual(values = c('variance' = 16, 'w.variance' = 15))+
  scale_linetype_manual(values = c('variance' = 'solid', 'w.variance' = 'dashed'))
  

ggsave('figures/supp_figA6.png', supp_figA6)
ggsave('figures/supp_figA6.pdf', supp_figA6)



supp_figA7 = ggplot(filter(e0_var_sens, measure %in% c('theil', 'w.theil')), 
                    aes(x = year, y = dispersion, group = measure, color = measure))+
  geom_point(aes(shape = measure), show.legend = F)+
  geom_smooth(aes(linetype = measure), se = FALSE, show.legend = F)+
  xlab("Year")+
  ...
  theme_bw()+
  facet_wrap(. ~ str_to_title(sex), scales = 'free')+
  scale_color_manual(values = c('theil' = 'black', 'w.theil' = 'red'))+
  scale_shape_manual(values = c('theil' = 16, 'w.theil' = 15))+
  scale_linetype_manual(values = c('theil' = 'solid', 'w.theil' = 'dashed'))+
  scale_y_continuous(labels = scales::comma)

ggsave('figures/supp_figA7.png', supp_figA7)
ggsave('figures/supp_figA7.pdf', supp_figA7)
